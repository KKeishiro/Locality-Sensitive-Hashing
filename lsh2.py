...
def mapper(key, value):
    ...
    # Random hash function will take the form of:
    #   h(x) = (a*x + b) % c
    ...
    # just hash the IDs of the shingles that are *actually in the document*,
    # then take the lowest resulting hash code value. This corresponds to the index
    # of the first shingle that we would have encountered in the random order.
    ...
    for band_id, band in enumerate(band_signature):
        ...
        # yield bucket, {'page_id':page_id, 'shingle':value.split()[1:]}
        yield bucket, (page_id, value.split()[1:])
...
def reducer(key, values):
    ...
    if len(values) >= 2:
        ...
        for i in range(numPair):
            shingles1 = values[i][1]

            for j in range(i+1, numPair):
                shingles2 = values[j][1]

              # Calculate the Jaccard similality.
                J = float(len(set(shingles1).intersection(shingles2))) / len(set(shingles1).union(shingles2))
                if J >= threshold:
                    final_pair = ([values[i][0], values[j][0]])
                    yield int(sorted(final_pair,key=int)[0]), int(sorted(final_pair,key=int)[1])
